...
Generator.prototype.addJavaScriptToIndex = function (script) {
    try {
        ...
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + script + '.js ' + chalk.yellow('not added.\n'));
    }
};
...
Generator.prototype.addElementToMenu = function (routerName, glyphiconName, enableTranslation) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-add-element-to-menu',
            splicable: [
        ...
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + routerName + ' ' + chalk.yellow('not added to menu.\n'));
    }
};
...
Generator.prototype.addElementToAdminMenu = function (routerName, glyphiconName, enableTranslation) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-add-element-to-admin-menu',
            splicable: [
                    '<li ui-sref-active="active" ><a ui-sref="' + routerName + '" data-toggle="collapse" data-target=".navbar-collapse.in"><span class="glyphicon glyphicon-' + glyphiconName + '"></span>\n' +
        ...
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + routerName + ' ' + chalk.yellow('not added to admin menu.\n'));
    }
};
...
Generator.prototype.addEntityToMenu = function (routerName, enableTranslation) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-add-entity-to-menu',
            splicable: [
...
Generator.prototype.addElementTranslationKey = function(key, value, language) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-menu-add-element',
            splicable: [
                    '"' + key + '": "' + value + '",'
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + language + chalk.yellow(' not added as a new entity in the menu.\n'));
    }
};
...
Generator.prototype.addBowerrcParameter = function(key, value) {
    ...
    try {
        ...
        ashUtils.rewriteJSONFile(fullPath, function(jsonObj) {
            jsonObj[key] = value;
        });
    } catch (e) {
...
Generator.prototype.addAdminElementTranslationKey = function(key, value, language) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-menu-add-admin-element',
            splicable: [
                    '"' + key + '": "' + value + '",'
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + language + chalk.yellow(' not added as a new entry in the admin menu.\n'));
    }
};
...
Generator.prototype.addEntityTranslationKey = function(key, value, language) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-menu-add-entry',
            splicable: [
                    '"' + key + '": "' + value + '",'
            ]
        });
...
Generator.prototype.addSocialConfiguration = function(name, clientId, clientSecret, comment) {
    ...
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-add-social-configuration',
            splicable: [
                config
            ]
        });
...
Generator.prototype.addAngularJsModule = function(moduleName) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-angularjs-add-module',
            splicable: [
                "'" + moduleName + "',"
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + moduleName + chalk.yellow(' not added to JHipster app.\n'));
    }
};
...
Generator.prototype.addAngularJsConfig = function(moduleConfigNames, config, comment) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-angularjs-add-config',
            splicable: [
                configBlock
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Configuration not added to JHipster app.\n'));
    }
};
...
Generator.prototype.addAngularJsInterceptor = function(interceptorName) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-angularjs-add-interceptor',
            splicable: [
                '$httpProvider.interceptors.push(\'' + interceptorName + '\');'
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Interceptor not added to JHipster app.\n'));
    }
};
...
Generator.prototype.addChangelogToLiquibase = function (changelogName) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-liquibase-add-changelog',
            splicable: [
                    '<include file="classpath:config/liquibase/changelog/' + changelogName + '.xml" relativeToChangelogFile="false"/>'
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + changelogName + '.xml ' + chalk.yellow('not added.\n'));
    }
};
...
Generator.prototype.addColumnToLiquibaseEntityChangeset = function (filePath, content) {
    try {
        ashUtils.rewriteFile({
            file: filePath,
            needle: 'ash-needle-liquibase-add-column',
            splicable: [
                content
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + filePath + chalk.yellow(' or missing required ash-needle. Column not added.\n') + e);
    }
};
...
Generator.prototype.addSocialButton = function (socialName, socialParameter, buttonColor, buttonHoverColor) {
    ...
    try {
        ...
        ashUtils.rewriteFile({
            ...
            needle: 'ash-needle-add-social-button',
            splicable: [
                serviceCode
            ]
        });
...
Generator.prototype.addSocialConnectionFactory = function (javaDir, importPackagePath, socialName, connectionFactoryClassName, configurationName) {
    ...
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-add-social-connection-factory-import-package',
            splicable: [
                javaImport
            ]
        });
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-add-social-connection-factory',
            splicable: [
                javaCode
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Social connection ') + e + ' ' + chalk.yellow('not added.\n'));
    }
};
...
Generator.prototype.addMainCSSStyle = function(style, comment) {
    // Not Working this.useSass -> Undifined --> this in the modules context would be ashFunc
    if (this.useSass) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-css-add-main',
            splicable: [
                styleBlock
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Style not added to JHipster app.\n'));
    }
};
...
Generator.prototype.addMainSCSSStyle = function(style, comment) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-scss-add-main',
            splicable: [
                styleBlock
            ]
        });
...
Generator.prototype.addMavenDependency = function (groupId, artifactId, version, other) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-maven-add-dependency',
            splicable: [
                dependency
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + 'maven dependency (groupId: ' + groupId + ', artifactId:' + artifactId + ', version:' + version + ')' + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addMavenPlugin = function (groupId, artifactId, version, other) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-maven-add-plugin',
            splicable: [
                plugin
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + 'maven plugin (groupId: ' + groupId + ', artifactId:' + artifactId + ', version:' + version + ')' + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addGradlePlugin = function (group, name, version) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-gradle-buildscript-dependency',
            splicable: [
                'classpath group: \'' + group + '\', name: \'' + name + '\', version: \'' + version + '\''
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + 'classpath: ' + group + ':' + name + ':' + version + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.addGradleDependency = function (scope, group, name, version) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-gradle-dependency',
            splicable: [
                scope + ' group: \'' + group + '\', name: \'' + name + '\', version: \'' + version + '\''
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + group + ':' + name + ':' + version + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.applyFromGradleScript = function (name) {
    try {
        ...
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-gradle-apply-from',
            splicable: [
                    'apply from: \'' + name + '.gradle\''
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + name + chalk.yellow(' not added.\n'));
    }
};
...
Generator.prototype.copyTemplate = function (source, dest, action, _this, _opt, template) {
    ...
    switch(action) {
        case 'stripHtml' :
            ...
            ashUtils.copyWebResource(source, dest, regex, 'html', _this, _opt, template);
            break;
        case 'stripJs' :
            var regex = '[a-zA-Z]+\:(\s)?\[[ \'a-zA-Z0-9\$\,\(\)\{\}\n\.\<\%\=\>\;\s]*\}\]';
            //looks for something like mainTranslatePartialLoader: [*]
            ashUtils.copyWebResource(source, dest, regex, 'js', _this, _opt, template);
            break;
        case 'copy' :
...
/**
 * Register a module configuration to .ash-modules.json
 *
...
Generator.prototype.registerModule = function(moduleConfig) {
    try {
        ...
        console.log('\n' + chalk.bold.red('Could not add ash module configuration' + err));
    }
};
...
Generator.prototype.installNewLanguage = function(language) {
    ...
    try {
        ashUtils.rewriteFile({
            file: fullPath,
            needle: 'ash-needle-006',
            splicable: [
                    ',\'' + language + '\''
            ]
        });
    } catch (e) {
        console.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required ash-needle. Reference to ') + language + chalk.yellow(' not added as a new language. Check if you have enabled translation support.\n'));
    }
};
