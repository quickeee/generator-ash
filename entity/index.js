...
var EntityGenerator = module.exports = function EntityGenerator(args, options, config) {
    ...
    this.ashConfigDirectory = '.ash';
    this.name = this.name.replace('.json','');
    this.filename = this.ashConfigDirectory + '/' + _s.capitalize(this.name) + '.json';
    ...
    if (this.useConfigurationFile == false) { // store informations in a file for further use.
        ...
        for (var idx in this.fields) {
            ...
            if (_.isUndefined(field.fieldId)) {
                console.log(chalk.red('ERROR fieldId is missing in .ash/' + this.name + '.json for field ' + JSON.stringify(field, null, 4)));
                process.exit(1);
            }

            if (_.isUndefined(field.fieldName)) {
                console.log(chalk.red('ERROR fieldName is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                process.exit(1);
            }

            if (_.isUndefined(field.fieldType)) {
                console.log(chalk.red('ERROR fieldType is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                process.exit(1);
            }

            if (!_.isUndefined(field.fieldValidateRules)) {
                if (!_.isArray(field.fieldValidateRules)) {
                    console.log(chalk.red('ERROR fieldValidateRules is not an array in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
                for (var idxRules in field.fieldValidateRules) {
                    ...
                    if (!_.contains(supportedValidationRules, fieldValidateRule)) {
                        console.log(chalk.red('ERROR fieldValidateRules contains unknown validation rule ' + fieldValidateRule + ' in .ash/' + this.name + '.json for field with id ' + field.fieldId + ' [supported validation rules ' + supportedValidationRules + ']'));
                        process.exit(1);
                    }
                }
                if (_.contains(field.fieldValidateRules, 'max') && _.isUndefined(field.fieldValidateRulesMax)) {
                    console.log(chalk.red('ERROR fieldValidateRulesMax is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
                if (_.contains(field.fieldValidateRules, 'min') && _.isUndefined(field.fieldValidateRulesMin)) {
                    console.log(chalk.red('ERROR fieldValidateRulesMin is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
                if (_.contains(field.fieldValidateRules, 'maxlength') && _.isUndefined(field.fieldValidateRulesMaxlength)) {
                    console.log(chalk.red('ERROR fieldValidateRulesMaxlength is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
                if (_.contains(field.fieldValidateRules, 'minlength') && _.isUndefined(field.fieldValidateRulesMinlength)) {
                    console.log(chalk.red('ERROR fieldValidateRulesMinlength is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
                if (_.contains(field.fieldValidateRules, 'maxbytes') && _.isUndefined(field.fieldValidateRulesMaxbytes)) {
                    console.log(chalk.red('ERROR fieldValidateRulesMaxbytes is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
                if (_.contains(field.fieldValidateRules, 'minbytes') && _.isUndefined(field.fieldValidateRulesMinbytes)) {
                    console.log(chalk.red('ERROR fieldValidateRulesMinbytes is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
                if (_.contains(field.fieldValidateRules, 'pattern') && _.isUndefined(field.fieldValidateRulesPattern)) {
                    console.log(chalk.red('ERROR fieldValidateRulesPattern is missing in .ash/' + this.name + '.json for field with id ' + field.fieldId));
                    process.exit(1);
                }
            }
        }
        ...
        for (var idx in this.relationships) {
            ...
            if (_.isUndefined(relationship.relationshipId)) {
                console.log(chalk.red('ERROR relationshipId is missing in .ash/' + this.name + '.json for relationship ' + JSON.stringify(relationship, null, 4)));
                process.exit(1);
            }

            if (_.isUndefined(relationship.relationshipName)) {
                relationship.relationshipName = relationship.otherEntityName;
                console.log(chalk.yellow('WARNING relationshipName is missing in .ash/' + this.name + '.json for relationship with id ' + relationship.relationshipId + ', using ' + relationship.otherEntityName + ' as fallback'));
            }

            if (_.isUndefined(relationship.otherEntityName)) {
                console.log(chalk.red('ERROR otherEntityName is missing in .ash/' + this.name + '.json for relationship with id ' + relationship.relationshipId));
                process.exit(1);
            }
            ...
            if (_.isUndefined(relationship.otherEntityRelationshipName)
                ...
                console.log(chalk.yellow('WARNING otherEntityRelationshipName is missing in .ash/' + this.name + '.json for relationship with id ' + relationship.relationshipId + ', using ' + _s.decapitalize(this.name) + ' as fallback'));
            }
            ...
            if (_.isUndefined(relationship.otherEntityField)
                ...
                console.log(chalk.yellow('WARNING otherEntityField is missing in .ash/' + this.name + '.json for relationship with id ' + relationship.relationshipId + ', using id as fallback'));
                relationship.otherEntityField = 'id';
            }

            if (_.isUndefined(relationship.relationshipType)) {
                console.log(chalk.red('ERROR relationshipType is missing in .ash/' + this.name + '.json for relationship with id ' + relationship.relationshipId));
                process.exit(1);
            }
            ...
            if (_.isUndefined(relationship.ownerSide)
                ...
                console.log(chalk.red('ERROR ownerSide is missing in .ash/' + this.name + '.json for relationship with id ' + relationship.relationshipId));
                process.exit(1);
            }
        }
        ...
        if (_.isUndefined(this.changelogDate)
            ...
            console.log(chalk.yellow('WARNING changelogDate is missing in .ash/' + this.name + '.json, using ' + currentDate + ' as fallback'));
            this.changelogDate = currentDate;
        }
        if (_.isUndefined(this.dto)) {
            console.log(chalk.yellow('WARNING dto is missing in .ash/' + this.name + '.json, using no as fallback'));
            this.dto = 'no';
        }
        if (_.isUndefined(this.service)) {
            console.log(chalk.yellow('WARNING service is missing in .ash/' + this.name + '.json, using no as fallback'));
            this.service = 'no';
        }
        if (_.isUndefined(this.pagination)) {
            if (databaseType == 'sql' || databaseType == 'mongodb') {
                console.log(chalk.yellow('WARNING pagination is missing in .ash/' + this.name + '.json, using no as fallback'));
                this.pagination = 'no';
            } else {
                this.pagination = 'no';
            }
